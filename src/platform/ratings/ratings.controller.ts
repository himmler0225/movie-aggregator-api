...
export class RatingsController {
  ...
  aggregate(@Param('slug') slug: string) {
    return this.ratings.getAggregate(slug);
  }

  @ApiBearerAuth()
  @Get('movie/:slug')
  userRating(@CurrentUser() user: AuthUser, @Param('slug') slug: string) {
    return this.ratings.getUserRating(user.id, slug);
  }

  @ApiBearerAuth()
  @Put('movie/:slug')
  upsert(@CurrentUser() user: AuthUser, @Param('slug') slug: string, @Body() body: UpsertRatingDto) {
