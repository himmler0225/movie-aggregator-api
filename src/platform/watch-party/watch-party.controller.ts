...
export class WatchPartyController {
  ...
  @ApiBearerAuth()
  @Post('rooms')
  create(@CurrentUser() user: AuthUser, @Body() body: CreateWatchRoomDto) {
  ...
  members(@Param('roomId') roomId: string) {
    return this.watchParty.fetchMembers(roomId);
  }

  @Public()
  @Get('rooms/:roomId/messages')
  messages(@Param('roomId') roomId: string) {
    return this.watchParty.fetchMessages(roomId);
  }

  @ApiBearerAuth()
  @Post('rooms/:roomId/join')
  join(@CurrentUser() user: AuthUser, @Param('roomId') roomId: string, @Body() body: JoinWatchRoomDto) {
  ...
  leave(@CurrentUser() user: AuthUser, @Param('roomId') roomId: string) {
    return this.watchParty.removeMember(roomId, user.id);
  }

  @ApiBearerAuth()
  @Delete('rooms/:roomId')
  close(@CurrentUser() user: AuthUser, @Param('roomId') roomId: string) {
