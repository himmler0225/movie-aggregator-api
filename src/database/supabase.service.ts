...
export class SupabaseService {
  ...
  constructor(
    ...
    if (!appConfig.isSupabaseConfigured) {
      this.logger.warn('SUPABASE_URL or SUPABASE_SERVICE_KEY missing — auth will fail');
      this.client = null;
      return;
    }
  ...
  updateUserById(userId: string, attrs: { password?: string; user_metadata?: Record<string, unknown> }) {
  ...
  resetPasswordForEmail(email: string, redirectTo: string) {
    return this.requireClient().auth.resetPasswordForEmail(email, { redirectTo });
  }
  ...
  async findUserByEmail(email: string): Promise<User | null> {
    ...
    try {
      ...
      this.logger.warn(`auth.users lookup failed, falling back to admin API: ${String(err)}`);
    }
