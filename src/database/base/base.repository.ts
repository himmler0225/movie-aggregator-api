...
/**
 * Abstract base repository — subclasses pass a Prisma model delegate
 * and inherit common CRUD / query helpers.
 */
export abstract class BaseRepository<TEntity = unknown> {
  ...
  findById(id: string, select?: Record<string, unknown>): Promise<TEntity | null> {
    return this.delegate.findUnique({ where: { id }, select });
  }

  findOne(
    where: Record<string, unknown>,
    opts?: { orderBy?: unknown; select?: Record<string, unknown> },
  ...
  async countSince(dateField: string, since: Date | string | null): Promise<number> {
    const where =
      since == null
        ? undefined
        : { [dateField]: { gte: since instanceof Date ? since : new Date(since) } };
  ...
  create(data: Record<string, unknown>, select?: Record<string, unknown>): Promise<TEntity> {
    return this.delegate.create({ data, select });
  }

  async createMany(data: Record<string, unknown>[], skipDuplicates = false): Promise<number> {
  ...
  async updateMany(where: Record<string, unknown>, data: Record<string, unknown>): Promise<number> {
